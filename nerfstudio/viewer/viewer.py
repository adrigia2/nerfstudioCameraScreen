...
class Viewer:
    ...
    def __init__(
        ...
        with test_tab:
            self.capture_images_panel = CaptureImagesPanel(self.viser_server, self)
    ...
    def handle_new_client(self, client: viser.ClientHandle) -> None:
        ...
        if self.capture_images_panel is not None:
            self.capture_images_panel.set_render_state_machine(
                self.render_statemachines[client.client_id]
            )
    ...
    def training_end(self) -> None:
        assert self.capture_images_panel is not None
        self.capture_images_panel.training_end()
